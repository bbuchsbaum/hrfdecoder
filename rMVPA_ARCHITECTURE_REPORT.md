...
This report documents the rMVPA plugin architecture with specific focus on how the **hrfdecoder** decoder model integrates. The analysis covers (1) the S3 model plugin system, (2) data flow through searchlight/cross-validation pipelines, (3) existing preprocessing capabilities, and (4) architectural patterns for extending the framework.
...
rMVPA uses S3 classes for extensibility. Core model operations dispatch through:
...
- `format_result(obj, result, error_message, context, ...)` → formats fold-level outputs
- `merge_results(obj, result_set, indices, id, ...)` → aggregates fold results to ROI-level performance
...
**Key Differences from Standard MVPA Models**:
- Operates on TR-level (continuous) data, not trial-level betas
- `y_train()` returns dummy TR sequence (1:T) for fold construction
- Actual targets come from `design$event_model` and `design$events` (ignored by fold machinery)
- Fold assignment determined by `block_var` (run IDs), not `y_train` values
...
**Key Insight**: Data extraction (step 1) happens **before** fold creation. This means:
- **Preprocessing MUST happen before fold iteration** to avoid data leakage
- Preprocessing can be done per-ROI, before step 2
- No per-fold preprocessing without custom handling
...
The extracted ROI data is wrapped in **modelr::resample** objects (lightweight index-based views):
...
**When Applied**: During dataset initialization in the configuration pipeline (not fold-aware).
...
**Timing**: Called per-fold **within** `internal_crossval` **after** fold split. Avoids feature selection bias.
...
- Normalization (`normalize_samples` flag in config) is limited to z-scoring
...
**For standard models** (e.g., `sda_notune`):
- Passed via `model` object in registry (MVPAModels)
- `fit(x, y, wts, param, lev, last, weights, classProbs, ...)` signature

**For hrfdecoder**:
- Passed as explicit function arguments: `lambda_W`, `lambda_HRF`, `lambda_smooth`, etc.
- Stored directly in model spec
- Accessed in `train_model.hrfdecoder_model()` via `obj$lambda_W`, etc.

### 4.3 Cross-Validation Interaction with Preprocessing

**Key Issue**: CV folds created **after** ROI extraction but preprocessing timing matters:
...
**For AR Prewhitening**:
1. Must happen **before** fold creation to avoid cross-fold contamination
2. Must use **training data only** per fold (inside `train_model`)
3. Decoder weights fit on prewhitened training data
4. Test predictions made on independently prewhitened test data
...
**Key**: Block-level CV ensures entire runs are held out (respects temporal structure).
...
Each method receives the full model_spec object, allowing access to all parameters.
...
| `mvpa_model.R` | Standard MVPA model spec | `mvpa_model()`, `create_model_spec()`, S3 methods |
| `hrfdecoder_model.R` | hrfdecoder adapter | `hrfdecoder_model()`, `train_model.hrfdecoder_model()`, `format_result.hrfdecoder_model()` |
...
| `crossval.R` | CV specification classes | `blocked_cross_validation()`, `crossval_samples()` |
...
| `searchlight.R` | Searchlight iteration | `run_searchlight()`, output wrapping |
| `regional.R` | Regional analysis | `run_regional()`, region mask handling |
| `dataset.R` | Dataset structures | `mvpa_dataset()`, `gen_sample_dataset()` |
| `design.R` | Design specifications | `mvpa_design()`, formula parsing |
| `classifiers.R` | Model registry | `MVPAModels` environment, `load_model()` |
...
- [ ] Ensure `format_result.hrfdecoder_model()` applies same AR transform to test data
- [ ] Validate that prewhitening happens before fold creation (no data leakage)
- [ ] Document AR parameter control in `hrfdecoder_model()` signature
- [ ] Test with cross-validation to ensure train/test independence
- [ ] Add `ar_order` parameter to `hrfdecoder_model()` spec
- [ ] Consider model-agnostic interface for other preprocessing (nuisance regression, etc.)
...
1. **Train/test separation**: Each fold's AR model estimated from training data only
2. **Consistency**: Test data transformed using training fold's AR parameters
3. **Model transparency**: AR fit stored in result object for inspection
4. **Flexibility**: Works with or without prewhitening
5. **Extensibility**: Same pattern works for other preprocessing (nuisance regression, etc.)
